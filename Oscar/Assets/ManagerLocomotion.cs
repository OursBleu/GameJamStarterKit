...
public class ManagerLocomotion : Manager, IDisableable
{
    ...
    public bool IsGrounded
    {
        get
        {
            ...
            Vector3 topLeft = col.bounds.center + Vector3.left * (col.bounds.extents.x) + Vector3.down * (col.bounds.extents.y);
            Vector3 bottomRight = col.bounds.center + Vector3.right * (col.bounds.extents.x) + Vector3.down * (col.bounds.extents.y + 0.1f);
            return Physics2D.OverlapArea(topLeft, bottomRight, 1 << LayerMask.NameToLayer("Ground")) != null;
        }
    }
    ...
    public void Move(bool ignoreDisabling = false)
    {
        Move(_defaultDirection, _defaultSpeed, ignoreDisabling);
    }

    public void Move(Vector2 direction, bool ignoreDisabling = false)
    {
        Move(direction, _defaultSpeed, ignoreDisabling);
    }
